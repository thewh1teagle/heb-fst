...
def get_hebrew_phonemizer_fst(fst_path: str):
    ...
    if _HEBREW_FST is None:
        if os.path.exists(fst_path):
            # Load from file
            _HEBREW_FST = pynini.Fst.read(fst_path)
        else:
            # Create new FST and save it
            _HEBREW_FST = create_phonemizer_fst()
            _HEBREW_FST.write(fst_path)
    
    return _HEBREW_FST


def create_phonemizer_fst():
